  ...
  const handleMicrosoftSignIn = useCallback(async () => {
    ...
    try {
      ...
      setDeviceAuthInfo({
        ...
        message: deviceJson.message,
      });

      const intervalMs = Math.max(5, Number(deviceJson.interval) || 5) * 1000;
      let tokenJson;
      ...
      console.error("Microsoft device flow failed:", err);
    } finally {
    ...
    <div className="page-container" style={{ paddingTop: 24 }}>
      ...
      <div className="panel" style={{ marginTop: 16 }}>
        ...
        {deviceAuthInfo && (
          ...
          >
            {deviceAuthInfo.verificationUri && (
              <p style={{ margin: 0 }}>
                Go to
                {" "}
                <a href={deviceAuthInfo.verificationUri} target="_blank" rel="noreferrer">
                  {deviceAuthInfo.verificationUri}
                </a>
                {" "}
                and enter code <strong>{deviceAuthInfo.userCode}</strong>.
              </p>
            )}
            {deviceAuthInfo.message && (
              <p style={{ marginTop: 8 }}>{deviceAuthInfo.message}</p>
            )}
          </div>
        )}
